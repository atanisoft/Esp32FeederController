...
static constexpr const char * const CHIP_MODEL_NAMES[] =
{
...
static constexpr const char * const TAG = "soc_info";

#define ARRAYSIZE(x)    (sizeof(x) / sizeof(x[0]))

uint8_t SocInfo::print_soc_info()
{
    ...
    if (chip_model > ARRAYSIZE(CHIP_MODEL_NAMES))
    {
        chip_model = 0;
    }

    ESP_LOGI(TAG,
        "Reset reason:%s (%d), model:%s, chip revision:%d, number of cores:%d, "
        "embedded flash:%s, embedded PSRAM: %s, WiFi:%s, IEEE 802.15.4:%s, "
        "BLE:%s, BT:%s",
        RESET_REASONS[reset_reason], reset_reason, CHIP_MODEL_NAMES[chip_model],
